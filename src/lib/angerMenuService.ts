...
export interface AngerMenuSession {
  id?: string
  user_id?: string
  video_id: string
  video_title: string
  ...
  selected_techniques?: string[] // técnicas seleccionadas por el usuario
...
export const angerMenuService = {
  // Obtener o crear la sesión única del usuario para un video
  async getOrCreateSession(userId: string, videoId: string, videoTitle: string): Promise<AngerMenuSession | null> {
    if (!supabase) {
    ...
    try {
      ...
      if (fetchError && fetchError.code === 'PGRST116') { // No rows returned
        ...
        if (createError) {
          console.error('Error creating anger menu session:', createError)
          return null
        }
  ...
  async updateSession(userId: string, videoId: string, videoTitle: string, updates: Partial<AngerMenuSession>): Promise<AngerMenuSession | null> {
    if (!supabase) {
      console.warn('Supabase not configured')
      return null
    }
    ...
    try {
      ...
      if (error) {
        console.error('Error updating anger menu session:', error)
        return null
      }
  ...
  async getAllSessions(userId: string): Promise<AngerMenuSession[]> {
    if (!supabase) {
    ...
    try {
      ...
      if (error) {
        console.error('Error fetching anger menu sessions:', error)
        return []
      }
